...
void main() {
 ...
 #if isoRenderFlag==0
  {
      ...
      if (vol > t_function2min.a)
        ...
        acc.rgb = volumeOpacity*VolumeRender(start.xyz  + max(0., minIso.a - 0. / 128.)*dir , dir, back, true).rgb;
      acc.a = 1.0;

      // float mx = MipMax(start.xyz, dir, back);
      // if (mx > 0.98)
      //   acc.rgb = vec3(1.0, 1.0, 0.0); //yellow
      // else
  ...
  #endif
  // Render of maximum intensity or a overlay for volumetric
  #if isoRenderFlag == 2
  {
